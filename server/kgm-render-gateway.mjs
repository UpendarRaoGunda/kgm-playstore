...
import { getKgmLocalUser, handleKgmLocalApi } from "./kgm-local-api.mjs";
...
const json = (res, status, value) => { const body = Buffer.from(JSON.stringify(value)); res.writeHead(status, { "content-type": "application/json; charset=utf-8", "content-length": body.length, "cache-control": "no-store" }); res.end(body); };

async function curator(req) {
  const user = getKgmLocalUser(req);
  return user?.role === "Adult" ? user : null;
}
...
async function handle(req, res) {
  ...
  if (url.pathname === "/api/kgm-media/health") return json(res, 200, { status: "ok", service: "kgm-playstore", storage: STORAGE, disk: existsSync(STORAGE) });
  ...
  if (url.pathname === "/api/kgm-cinema/admin/youtube" && req.method === "POST") {
    const who = await curator(req); if (!who) return json(res, 403, { detail: "KGM curator access required" });
    const body = await new Response(Readable.toWeb(req)).json().catch(() => ({}));
    if (!body.stem_confirmed || !body.embed_confirmed) return json(res, 400, { detail: "Confirm STEM relevance and YouTube embed availability" });
    if (!body.youtube_id && !body.youtube_playlist_id) return json(res, 400, { detail: "Add a YouTube video or playlist" });
    const item = { id: `youtube-${body.youtube_id || crypto.randomUUID()}`, source: body.youtube_playlist_id && !body.youtube_id ? "youtube_playlist" : "youtube", youtube_id: body.youtube_id || null, youtube_playlist_id: body.youtube_playlist_id || null, title: String(body.title || "STEM film").slice(0, 120), description: String(body.description || "").slice(0, 600), category: String(body.category || "Science").slice(0, 40), channel: String(body.channel || "").slice(0, 100), age_rating: String(body.age_rating || "All ages").slice(0, 30), duration_label: String(body.duration_label || "").slice(0, 40), language: String(body.language || "English").slice(0, 40), attribution: String(body.attribution || "YouTube").slice(0, 120), source_page: String(body.source_page || "").slice(0, 300), topics: Array.isArray(body.topics) ? body.topics.slice(0, 12) : [], learn: Array.isArray(body.learn) ? body.learn.slice(0, 8) : [], created_at: new Date().toISOString(), curator_id: who.id, like_count: 0 };
    const items = catalog().filter((x) => x.id !== item.id); items.unshift(item); saveJson(CATALOG, items); return json(res, 201, item);
  }
  ...
  if (mediaMatch) {
    ...
    if (range) { const [s, e] = range.replace(/bytes=/, "").split("-"); const start = Number(s || 0), end = Math.min(Number(e || total - 1), total - 1); res.writeHead(206, { "content-type": item.content_type, "accept-ranges": "bytes", "content-range": `bytes ${start}-${end}/${total}`, "content-length": end - start + 1, "content-disposition": mediaMatch[2] === "download" ? `attachment; filename="${safeName(item.title)}.${item.content_type === "video/webm" ? "webm" : "mp4"}` : "inline" }); createReadStream(path, { start, end }).pipe(res); return; }
    res.writeHead(200, { "content-type": item.content_type, "content-length": total, "accept-ranges": "bytes", "content-disposition": mediaMatch[2] === "download" ? `attachment; filename="${safeName(item.title)}.${item.content_type === "video/webm" ? "webm" : "mp4"}` : "inline" }); createReadStream(path).pipe(res); return;
  }

  if (url.pathname === "/api/kgm-cinema/movies" && req.method === "GET") {
    const local = catalog().filter((x) => ["render", "youtube", "youtube_playlist"].includes(x.source)).map((x) => x.source === "render" ? localMovie(x) : x);
    ...
    const combined = [...PINNED_STEM_MOVIES, ...local].filter((item, index, all) => all.findIndex((x) => x.id === item.id) === index).filter(matches);
    const featured = combined.filter((x) => x.featured); const rest = combined.filter((x) => !x.featured);
    return json(res, 200, { items: [...featured, ...rest], categories: [...new Set(["All", ...combined.map((x) => x.category).filter(Boolean)])] });
  }

  if (url.pathname.startsWith("/api/kgm-")) {
    const handled = await handleKgmLocalApi(req, res, url);
    if (handled) return;
    return json(res, 404, { detail: "KGM API route not found" });
  }
...
http.createServer((req, res) => handle(req, res).catch((err) => { console.error(err); json(res, 500, { detail: "KGM gateway error" }); })).listen(PORT, "0.0.0.0", () => console.log(`KGM gateway :${PORT}; Vinext :${APP_PORT}; storage ${STORAGE}`));
